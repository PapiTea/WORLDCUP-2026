    ...
    <main className="min-h-screen px-4 pb-28 pt-6 md:pl-28 md:pr-8 md:pb-10">
      <section className="mx-auto max-w-7xl">
        <div className="mb-8 overflow-hidden rounded-[2rem] border border-white/10 bg-card/70 p-6 shadow-2xl backdrop-blur md:p-8">
          ...
          <h1 className="font-headline text-4xl font-black tracking-tight md:text-5xl">Match Predictions</h1>
        ...
        <Tabs defaultValue="group" className="space-y-6">
          <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            <TabsList className="h-12 rounded-2xl bg-muted/70 p-1">
              <TabsTrigger value="group" className="rounded-xl px-5 font-black data-[state=active]:bg-primary data-[state=active]:text-primary-foreground">Group Stage</TabsTrigger>
              <TabsTrigger value="knockouts" className="rounded-xl px-5 font-black data-[state=active]:bg-primary data-[state=active]:text-primary-foreground">Knockouts</TabsTrigger>
            </TabsList>
            <div className="relative w-full md:w-80">
              <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-muted-foreground" size={16} />
              <Input className="h-12 rounded-2xl bg-card/70 pl-11" placeholder="Search coming later..." />
            </div>
          </div>

          <TabsContent value="group" className="grid gap-5 lg:grid-cols-2 2xl:grid-cols-3">
            {MATCHES.map((match) => (
          ...
          <TabsContent value="knockouts" className="space-y-8">
            <div className="rounded-[2rem] border border-white/10 bg-card/70 p-5 shadow-xl backdrop-blur md:p-6">
              <h2 className="font-headline text-2xl font-black">Knockout predictions</h2>
            ...
            {KNOCKOUT_ROUNDS.map((round) => {
              const fixtures = KNOCKOUT_FIXTURES.filter((fixture) => fixture.round === round.id)
              return (
                <section key={round.id} className="space-y-4">
                  <div>
                    <h3 className="font-headline text-xl font-black md:text-2xl">{round.name}</h3>
                    <p className="text-sm text-muted-foreground">{round.helper}</p>
                  </div>
                  <div className="grid gap-5 lg:grid-cols-2 2xl:grid-cols-3">
                    {fixtures.map((fixture) => <KnockoutMatchCard key={fixture.id} fixture={fixture} />)}
                  </div>
                </section>
              )
            })}
