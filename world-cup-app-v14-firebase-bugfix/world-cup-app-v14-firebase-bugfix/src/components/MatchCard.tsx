...
import { saveMatchPrediction, subscribeUserMatchPredictions } from "@/lib/firebase-service"
...
type ScoreState = { homeScore: string; awayScore: string; saved: boolean }

function readConfidenceIds(): string[] {
  if (typeof window === "undefined") return []
  try {
    const parsed = JSON.parse(window.localStorage.getItem(CONFIDENCE_KEY) || "[]")
    return Array.isArray(parsed) ? Array.from(new Set(parsed.map(String))).slice(0, CONFIDENCE_LIMIT) : []
  } catch {
    return []
  }
}
...
export function MatchCard({ match, onSave }: MatchCardProps) {
  ...
  const scoreRef = useRef<ScoreState>({ homeScore: "", awayScore: "", saved: false })
  ...
  useEffect(() => {
    ...
    if (savedPick) {
      try {
        ...
        setScoreState({ homeScore: pick.home === undefined ? "" : String(pick.home), awayScore: pick.away === undefined ? "" : String(pick.away), saved: true })
      } catch {}
    } else {
      setScoreState({ homeScore: "", awayScore: "", saved: false })
    }
  ...
  useEffect(() => {
    ...
    const unsub = subscribeUserMatchPredictions(user.uid, (items) => {
      ...
      if (current.homeScore !== nextHome || current.awayScore !== nextAway || !current.saved) {
        setScoreState({ homeScore: nextHome, awayScore: nextAway, saved: true })
        window.localStorage.setItem(`wc-match-${match.id}`, JSON.stringify({ home: remote.home, away: remote.away, confidence: Boolean(remote.confidence) }))
      }
      if (remote.confidence) {
        const ids = readConfidenceIds()
        if (!ids.includes(match.id)) writeConfidenceIds([...ids, match.id])
      }
    })
    return () => unsub()
  }, [user, match.id])
  ...
  const persistCurrentScore = async (confidenceOverride?: boolean) => {
    ...
    if (h === "" || a === "") return
    const pick = { home: Number(h), away: Number(a), confidence: confidenceOverride ?? readConfidenceIds().includes(match.id) }
  ...
  const onHomeChange = (value: string) => setScoreState({ homeScore: value, saved: false })
  const onAwayChange = (value: string) => setScoreState({ awayScore: value, saved: false })
    ...
    <Card className="overflow-hidden rounded-[1.75rem] border border-white/10 bg-card/75 p-4 shadow-xl backdrop-blur transition hover:-translate-y-0.5 hover:shadow-2xl sm:p-5">
      ...
      <div className="grid min-w-0 grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)] items-start gap-2 sm:gap-3">
        ...
        <div className="mt-5 flex shrink-0 items-center justify-center gap-1.5 rounded-2xl bg-background/55 p-1.5 ring-1 ring-border sm:mt-6 sm:gap-2 sm:p-2">
          <Input type="number" inputMode="numeric" placeholder="-" value={homeScore} onChange={(e) => onHomeChange(e.target.value)} disabled={isLocked} className="h-10 w-10 rounded-xl border-border bg-card p-0 text-center text-base font-black sm:h-11 sm:w-12 sm:text-lg" />
          <span className="font-black text-muted-foreground">:</span>
          <Input type="number" inputMode="numeric" placeholder="-" value={awayScore} onChange={(e) => onAwayChange(e.target.value)} disabled={isLocked} className="h-10 w-10 rounded-xl border-border bg-card p-0 text-center text-base font-black sm:h-11 sm:w-12 sm:text-lg" />
        </div>
      ...
      {isLocked ? (
        <div className="mt-4 flex items-center justify-center gap-2 rounded-2xl border border-dashed border-border bg-background/35 px-3 py-3 text-center text-xs font-bold text-muted-foreground">
          <Lock size={14} /> Predictions locked for this match.
        </div>
      ) : (
        <div className="mt-4 space-y-2.5">
          <div className="grid grid-cols-2 gap-2">
            <Button
              ...
              className={cn("h-11 rounded-2xl font-black", isConfidencePick && "shadow-lg shadow-primary/20")}
            ...
            >
              ...
              {isConfidencePick ? "Confidence picked" : confidenceLimitReached ? "Limit reached" : "Confidence x2"}
            </Button>
