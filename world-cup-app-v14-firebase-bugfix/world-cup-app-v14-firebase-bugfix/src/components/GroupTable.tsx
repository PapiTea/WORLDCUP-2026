...
export function GroupTable({ group }: GroupTableProps) {
  ...
  const persistSelected = async (next: string[]) => {
    ...
    if (user) await saveGroupPrediction(user.uid, group.id, next)
  }

  const toggleTeam = (teamId: string) => {
    const current = selectedRef.current
    let next: string[]
    if (current.includes(teamId)) next = current.filter((id) => id !== teamId)
    else if (current.length >= 2) next = [current[1], teamId]
