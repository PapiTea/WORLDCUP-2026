...
import { Home, LayoutGrid, Trophy, Users, Award, ShieldCheck } from 'lucide-react';
...
export function BottomNav() {
  ...
  return (
    <>
      <div className="fixed right-4 top-4 z-50 md:hidden"><ThemeToggle /></div>
      <nav className="fixed bottom-0 left-0 right-0 z-50 border-t border-white/5 bg-background/85 backdrop-blur-xl pb-safe md:hidden">
        <div className="flex h-16 items-center justify-around px-2">
          {navItems.slice(0, 5).map((item) => <NavLink key={item.name} item={item} active={pathname === item.href} />)}
        </div>
      </nav>
      ...
      <aside className="fixed bottom-0 left-0 top-0 z-50 hidden w-[88px] flex-col border-r border-white/5 bg-background/85 px-2 py-5 backdrop-blur-xl md:flex">
        <Link href="/" className="mb-6 flex flex-col items-center gap-1 rounded-2xl p-2 text-center text-primary">
          <Trophy size={26} />
          <span className="text-[10px] font-black uppercase leading-tight">World Cup App</span>
        </Link>
            ...
            return (
              <Link
                ...
                className={cn(
                  ...
                  isActive ? "bg-primary/15 text-primary shadow-sm shadow-primary/10" : "text-muted-foreground hover:bg-white/5 hover:text-foreground"
                )}
              >
...
function NavLink({ item, active }: { item: typeof navItems[number]; active: boolean }) {
  ...
  return (
    ...
    >
      <div className={cn("rounded-xl p-1.5 transition-all duration-300", active && "bg-primary/10")}>
