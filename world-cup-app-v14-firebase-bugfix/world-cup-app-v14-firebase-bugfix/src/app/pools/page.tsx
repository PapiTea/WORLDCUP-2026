...
import { createLeague, deleteLeague, joinLeague, leaveLeague, subscribeLeagues, subscribeMyLeagueMemberships, type League } from '@/lib/firebase-service'
...
export default function PoolsPage() {
  ...
  useEffect(() => {
    if (!user) return
    const unsubMembers = subscribeMyLeagueMemberships(user.uid, (items) => setMemberships(items.map((m) => m.leagueId)))
    const unsubLeagues = subscribeLeagues(setLeagueMap)
    return () => { unsubMembers(); unsubLeagues() }
  }, [user])
  ...
  useEffect(() => {
    ...
    if (league) joinLeague(league.toUpperCase(), user.uid)
  }, [user])

  const pools = useMemo(() => memberships.map(code => leagueMap[code]).filter(Boolean), [memberships, leagueMap])
  ...
  const saveProfile = async () => {
    ...
    const next: UserProfile = { actualName: cleanActual, displayName: cleanDisplay, avatar }
    await setDoc(doc(db, 'users', user.uid), { ...next, email: user.email, updatedAt: serverTimestamp() }, { merge: true })
  ...
  const removePool = async (pool: League) => {
    ...
    if (!ok) return
    if (isCreator) await deleteLeague(pool.code)
    else await leaveLeague(pool.code, user.uid)
  }
  ...
  return (
    <div className="min-h-screen pb-24 md:pb-8 pt-6 px-4 md:pl-28 md:pr-8 max-w-lg mx-auto md:max-w-4xl">
      <header className="mb-8">
        ...
        <p className="text-muted-foreground text-sm">Create separate live mini-leagues for colleagues, friends or family. Invite links now work across devices.</p>
      </header>
      ...
      <section className="grid gap-4 md:grid-cols-2 mb-8">
        ...
        <Card className="glass-card p-4 space-y-3">
          ...
          <Button variant="outline" className="w-full h-11 border-border bg-card hover:bg-muted" onClick={joinPool}>Join league</Button>
        </Card>
      </section>
            ...
            <Card key={pool.id} className="glass-card p-4">
              <div className="flex justify-between items-start mb-4">
                <div className="flex gap-4">
                  ...
                  <div>
                    <h3 className="font-headline font-bold text-lg leading-tight flex items-center gap-2">{pool.name}{isCreator && <Crown size={14} className="text-secondary" />}</h3>
                    <p className="text-xs text-muted-foreground font-medium">Code: {pool.code}</p>
                    <p className="mt-1 flex items-center gap-1 text-[10px] font-bold uppercase tracking-widest text-muted-foreground">{isCreator ? <><ShieldCheck size={10} /> League owner</> : 'Joined league'}</p>
                  </div>
                </div>
                <div className="flex items-center gap-1">
                  <Button size="icon" variant="ghost" className="text-muted-foreground hover:text-primary" title="Copy invite link" onClick={() => copyLink(pool)}>{copied === pool.code ? <Copy size={18} /> : <Share2 size={18} />}</Button>
                  <Button size="icon" variant="ghost" className="text-muted-foreground hover:bg-destructive/10 hover:text-destructive" title={isCreator ? 'Delete league' : 'Leave league'} onClick={() => removePool(pool)}>{isCreator ? <Trash2 size={18} /> : <UserMinus size={18} />}</Button>
                </div>
              </div>
              <div className="flex items-center gap-2 bg-muted/40 rounded-xl p-3 text-xs text-muted-foreground"><LinkIcon size={14} /><span className="truncate">{typeof window !== 'undefined' ? `${window.location.origin}/pools?league=${pool.code}` : pool.code}</span></div>
            </Card>
          )
        })}
      </div>
