...
export default function AdminPage() {
  ...
  useEffect(() => {
    ...
    return () => { unsubResults(); unsubSetup() }
  }, [isAdmin])
  ...
  const saveScore = async (matchId: string) => {
    ...
    await saveMatchResult(matchId, { home: Number(score.home), away: Number(score.away) })
    setSaved(true)
  }
  ...
  const saveKoScore = async (matchId: string) => {
    ...
    await saveMatchResult(`ko_${matchId}`, { home: Number(score.home), away: Number(score.away) })
    setSaved(true)
  ...
  const toggleQualifier = async (groupId: string, teamId: string) => {
    ...
    setQualifiers(prev => ({ ...prev, [groupId]: next }))
      ...
      <main className="min-h-screen px-4 pb-28 pt-6 md:pl-28 md:pr-8 md:pb-10">
        <section className="mx-auto max-w-2xl">
          <Card className="glass-card p-8 text-center">
            ...
            <p className="mt-2 text-sm text-muted-foreground">Only trahbar23@gmail.com can update scores, qualifiers and knockout setup.</p>
          </Card>
        </section>
        <BottomNav />
      </main>
      ...
      <section className="mx-auto max-w-7xl space-y-8">
        ...
        <section className="space-y-4">
          <div>
            ...
            <p className="text-sm text-muted-foreground">Enter scores here after each match finishes. These feed the fixture pages and live leaderboard scoring.</p>
          </div>
